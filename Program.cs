...
namespace BuildNeuralNetworkDemo
{
    class NeuralNetworkProgram
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Creating 70% training, 15% test and 15% validation data matrices");
            double[][] trainData = null;
            double[][] testData = null;
            double[][] validationData = null;
            MakeTrainTest(allData, out trainData, out testData, out validationData);
            ...
            Console.WriteLine("First 3 rows of validation data:");
            ShowMatrix(validationData, 3, 1, true);


            Normalize(trainData, new int[] { 0, 1, 2, 3 });
            Normalize(testData, new int[] { 0, 1, 2, 3 });
            Normalize(validationData, new int[] { 0, 1, 2, 3 });

            Console.WriteLine("\nFirst 5 rows of normalized training data:");
            ShowMatrix(trainData, 20, 1, true);
            Console.WriteLine("First 3 rows of normalized test data:");
            ShowMatrix(testData, 20, 1, true);
            ...
            nn.Train(trainData, validationData, maxEpochs, learnRate, momentum, weightDecay);
            ...
            double validationAcc= nn.Accuracy(testData);
            Console.WriteLine("\nAccuracy on test data = " + validationAcc.ToString("F4"));

            Console.WriteLine("\nEnd Build 2013 neural network demo\n");

            double[] newSample = new double[] { 5.2, 3.6, 1.5, 0.3 }; // Setosa
            double[] newSample2 = new double[] { 6.2, 3.2, 5.9, 2.4 }; // Virginica
            int prediction = nn.Predict(newSample);
            int prediction2 = nn.Predict(newSample2);

            // Output prediction result
            Console.WriteLine("Predicted Iris Species: " + (prediction == 0 ? "Iris Setosa" : prediction == 1 ? "Iris Versicolor" : "Iris Virginica"));
            Console.WriteLine("Predicted Iris Species: " + (prediction2 == 0 ? "Iris Setosa" : prediction2 == 1 ? "Iris Versicolor" : "Iris Virginica"));


        } // Main

        static void MakeTrainTest(double[][] allData, out double[][] trainData, 
                        out double[][] testData, out double[][] validationData)
        {
            // split allData into 70% trainData and 15% testData 15% validationData
            Random rnd = new Random(0);
            int totRows = allData.Length;
            int numCols = allData[0].Length;

            //Hard coded 70-15-15 split
            int trainRows = (int)(totRows * 0.70); //70%
            int testRows = (int)(totRows * 0.15);  //15%
            int validationRows = totRows - trainRows - testRows; //15%

            trainData = new double[trainRows][];
            testData = new double[testRows][];
            validationData = new double[validationRows][];

            int[] sequence = new int[totRows]; // create a random sequence of indexes
            for (int i = 0; i < sequence.Length; ++i)
                sequence[i] = i;

            //Shuffling the placements using temp variables
            for (int i = 0; i < sequence.Length; ++i)
            {
            ...
            for (int i = 0; i < trainRows; ++i)
            {
                ...
                int idx = sequence[si++];
                Array.Copy(allData[idx], trainData[i], numCols);
            }
            ...
            for (int i = 0; i < testRows; ++i)
            {
                ...
                int idx = sequence[si++];
                Array.Copy(allData[idx], testData[i], numCols);
            }

            for (int i = 0; i < validationRows; ++i)
            {
                validationData[i] = new double[numCols];
                int idx = sequence[si++];
                Array.Copy(allData[idx], validationData[i], numCols);
            }
    ...
    public class NeuralNetwork
    {
        ...
        private double[] ComputeOutputs(double[] xValues)
        {
            ...
            for (int j = 0; j < numOutput; ++j)   // compute h-o sum of weights * h Outputs
                for (int i = 0; i < numHidden; ++i)
        ...
        public int Predict(double[] inputData)
        {
            if (inputData.Length != numInput)
                throw new Exception("Input data length mismatch with the number of inputs in the network");

            // Use the trained model to compute the outputs (predictions)
            double[] outputs = this.ComputeOutputs(inputData);

            // Determine the index of the maximum output value (winner-takes-all)
            int predictedIndex = MaxIndex(outputs);

            // Return the predicted class (0 = Iris setosa, 1 = Iris versicolor, 2 = Iris virginica)
        ...
        public void Train(double[][] trainData, double[][] validationData, int maxEpochs, double learnRate, double momentum, double weightDecay)
        {
            int epoch = 0;
            ...
            {
                Shuffle(sequence); // Randomly shuffle data
                for (int i = 0; i < trainData.Length; ++i)
                {
                    int idx = sequence[i];
                    Array.Copy(trainData[idx], xValues, numInput);
                    Array.Copy(trainData[idx], numInput, tValues, 0, numOutput);
                    ComputeOutputs(xValues); // Forward pass
                    UpdateWeights(tValues, learnRate, momentum, weightDecay); // Backpropagation
                }

                // Monitor loss and accuracy every 100 epochs



                // Replace MSE with CrossEntropyLoss in the training loop
                if (epoch % 100 == 0)
                {
                    double trainLoss = CrossEntropyLoss(trainData);
                    double validationLoss = CrossEntropyLoss(validationData);
                    double trainAcc = Accuracy(trainData);
                    double valAcc = Accuracy(validationData);
                    Console.WriteLine($"Epoch: {epoch}, Train Loss: {trainLoss}, Val Loss: {validationLoss}, Train Acc: {trainAcc}, Val Acc: {valAcc}");
                }
                ...
                //    double trainMSE = MeanSquaredError(trainData);
                //    double validationMSE = MeanSquaredError(validationData);
                //    double trainAcc = Accuracy(trainData);
                //    double valAcc = Accuracy(validationData);
                //    Console.WriteLine($"Epoch: {epoch}, Train MSE: {trainMSE:F4}, Validation MSE: {validationMSE:F4}, Train Acc: {trainAcc:F4}, Val Acc: {valAcc:F4}");
                //}
                epoch++;
            }
        }
        ...
        private double CrossEntropyLoss(double[][] trainData)
        {
            ...
            for (int i = 0; i < trainData.Length; ++i)
            {
                ...
                {
                    if (tValues[j] == 1.0)
                    { // Only sum for the correct class
                        sumCrossEntropy += -Math.Log(yValues[j]);
                    }
                }
            }

            // Return the average cross-entropy loss across all samples
            return sumCrossEntropy / trainData.Length;
        }
